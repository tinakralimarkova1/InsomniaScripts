...
public class ChooseController : MonoBehaviour
{
    ...
    public void SetupChoose(ChooseScene scene)
    {
        ...
        size.y = (scene.labels.Count ) * labelHeight;
    ...
    private float CalculateLabelPosition(int labelIndex, int labelCount)
    {
        if(labelCount %2 == 0)
        {
            ...
            {
                return labelHeight * (labelCount / 2 - labelIndex - 1) + labelHeight / 3;
            }
            else
            {
                return -2 * (labelHeight * (labelIndex - labelCount / 2) + labelHeight / 3);
            }
        }
        else
        {
